...
--
-- groups.created_by is `text` while group_members.user_id is `uuid`, so the
-- obvious `gm.user_id = g.created_by` is a type error with no implicit cast —
-- which aborted this migration partway, leaving the role column added and
-- nothing else. Compare as text so it works whichever way the id was stored.
update public.group_members gm
set role = 'owner'
from public.groups g
where gm.group_id = g.id
  and gm.user_id::text = g.created_by
  and gm.role <> 'owner';

-- A creator who left their own group leaves it with nobody who can rename,
-- remove members or delete it. One such group already exists. Fall back to the
-- earliest member so every group has exactly one owner.
update public.group_members gm
set role = 'owner'
where gm.user_id = (
  select gm2.user_id
  ...
  order by gm2.joined_at nulls last, gm2.user_id
  limit 1
)
and not exists (
