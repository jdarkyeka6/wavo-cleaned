...
## iOS business model
The submitted iOS app is free-only. It does not offer in-app purchases, prices, upgrade buttons, external purchase links, or unlock paid Wavo digital features purchased on another platform. Optional Wavo paid plans are web products and are not available or activated inside the submitted iOS app.
...
- Replaced the submitted App Store icon with the finalized Wavo icon and removed alternate icon registration from the App Store build.
...
- Kept the submitted native app free-only, with no paid digital feature unlocks or purchase calls to action.
