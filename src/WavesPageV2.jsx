...
async function signPosts(posts) {
...
async function getGroupPosts(spaces) {
  const groupIds = (spaces || []).map((space) => space.id).filter(Boolean)
  ...
  const spaceMap = Object.fromEntries((spaces || []).map((space) => [space.id, space]))
  const reactions = reactionsResult.data || []

  return posts.map((post) => ({
    ...post,
    author: profileMap[post.author_id],
    group: spaceMap[post.group_id],
...
function Composer({ open, onClose, userId, friends, spaces, onCreated, initialTab }) {
  const [caption, setCaption] = useState('')
  const [visibility, setVisibility] = useState(initialTab === 'groups' ? 'group' : 'friends')
  const [selected, setSelected] = useState([])
  const [groupId, setGroupId] = useState(spaces?.[0]?.id || '')
  ...
  useEffect(() => {
    if (open) {
      setVisibility(initialTab === 'groups' && spaces.length ? 'group' : 'friends')
      setGroupId(spaces?.[0]?.id || '')
      return
    }
    setCaption('')
  ...
  async function submit(e) {
    ...
    if (visibility === 'group' && !groupId) return setError('Choose a group.')
    ...
    try {
      if (visibility === 'group') {
        const { data, error: postError } = await supabase
          .from('posts')
          .insert({ author_id: userId, body: caption.trim(), visibility: 'group', group_id: groupId })
          .select('*')
          .single()
  ...
  const selectedGroup = spaces.find((space) => space.id === groupId)
      ...
      <form className="waves-composer" onSubmit={submit}>
        ...
        <label className="waves-field">
          ...
          <select value={visibility} onChange={(e) => setVisibility(e.target.value)}>
            ...
            {spaces.length > 0 && <option value="group">A group</option>}
          </select>
        </label>

        {visibility === 'group' && (
          <label className="waves-field waves-group-field">
            <span>Group</span>
            <select value={groupId} onChange={(e) => setGroupId(e.target.value)}>
              {spaces.map((space) => <option key={space.id} value={space.id}>{space.emoji || '🌊'} {space.name}</option>)}
            </select>
            <small>Only current members of this group can see this Wave.</small>
          </label>
        )}
        ...
        <footer>
          <span>
            {visibility === 'friends' && 'Only people you have added can see this.'}
            {visibility === 'selected' && `${selected.length} selected`}
            {visibility === 'group' && `${selectedGroup?.emoji || '🌊'} ${selectedGroup?.name || 'Group'} members only`}
          </span>
...
function ReplyBox({ post, userId, onClose }) {
  ...
  async function submit(e) {
    ...
    setBusy(true)
    try {
      const prefix = post.body
        ? `↪ ${post.body.slice(0, 90)}${post.body.length > 90 ? '…' : ''}\n`
        : '↪ Replied to a Wave\n'
      if (post.visibility === 'group' && post.group_id) {
        await sendSpaceMessage(post.group_id, userId, `${prefix}${clean}`)
      } else {
        await sendDmMessage(userId, post.author_id, `${prefix}${clean}`)
      }
      onClose(post.visibility === 'group' ? 'Reply sent to group' : 'Reply sent')
    } catch {
      setBusy(false)
    }
  }

  const target = post.visibility === 'group'
    ? `${post.group?.emoji || '🌊'} ${post.group?.name || 'group'}`
    : post.author?.username

  return (
    <div className="waves-modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <form className="waves-reply" onSubmit={submit}>
        <header><strong>Reply in {target}</strong><button type="button" onClick={() => onClose()}><X size={18} /></button></header>
        <div className="waves-reply-context">{post.body || (post.media_type === 'video' ? 'Video Wave' : 'Photo Wave')}</div>
        <div className="waves-reply-row"><input autoFocus value={text} onChange={(e) => setText(e.target.value)} placeholder="Reply in Wavo messaging…" /><button disabled={busy || !text.trim()}><Send size={17} /></button></div>
      </form>
    </div>
  )
}
...
function WaveCard({ post, userId, onReply, onDelete, onReact }) {
  ...
  const audience = post.visibility === 'group'
    ? `${post.group?.emoji || '🌊'} ${post.group?.name || 'Group'}`
    : post.visibility === 'selected' ? 'Selected people' : 'Friends'
  ...
  return (
    <article className="waves-card">
      <header>
        ...
        <div>
          <strong>{post.author?.username || 'Wavo user'}</strong>
          <span>{relativeTime(post.created_at)} · {audience}</span>
        </div>
        {mine ? <button onClick={() => onDelete(post)} aria-label="Delete Wave"><Trash2 size={16} /></button> : <span className="waves-card-spacer" />}
      </header>

      {post.visibility === 'group' && post.group && (
        <div className="waves-group-chip">{post.group.emoji || '🌊'} {post.group.name}</div>
      )}
      ...
      <footer>
        {!mine && (
          <button className="waves-reply-button" onClick={() => onReply(post)}>
            <MessageCircle size={16} /> {post.visibility === 'group' ? 'Reply in group' : 'Reply'}
          </button>
        )}
        <div className="waves-reactions" aria-label="React to Wave">
          {['❤️', '😂', '💀', '🔥'].map((emoji) => (
            <button key={emoji} className={myReaction === emoji ? 'selected' : ''} onClick={() => onReact(post.id, emoji)}>{emoji}</button>
          ))}
        </div>
      </footer>
    </article>
...
export default function WavesPageV2() {
  ...
  async function refresh() {
    if (!userId) return
    const [nextFriends, nextSpaces] = await Promise.all([getFriends(userId), getSpaces(userId)])
    const [nextFriendPosts, nextGroupPosts] = await Promise.all([
      getPosts(userId, nextFriends),
      getGroupPosts(nextSpaces),
    ])

    setFriends(nextFriends)
    setSpaces(nextSpaces)
    setFriendPosts(await signPosts(nextFriendPosts.filter((post) => post.visibility !== 'group')))
    setGroupPosts(await signPosts(nextGroupPosts))
  }
  ...
  const activePosts = tab === 'groups' ? groupPosts : friendPosts
  const visiblePosts = useMemo(() => activePosts.slice(0, visibleCount), [activePosts, visibleCount])
  ...
  const intro = tab === 'groups'
    ? 'Updates shared inside groups you already belong to. Replies go straight back into the group conversation.'
    : 'Photos, short videos and updates shared only with friends or people you choose.'
  ...
  return (
    <main className="waves-page">
      ...
      <section className="waves-intro">
        <div><span>YOUR PEOPLE</span><h1>Waves</h1><p>{intro}</p></div>
        <button onClick={() => setComposerOpen(true)}><Plus size={19} /> Share</button>
      </section>

      <nav className="waves-tabs" aria-label="Wave feeds">
      ...
      <section className="waves-feed">
        {visiblePosts.length ? visiblePosts.map((post) => (
          ...
          <div className="waves-empty">
            {tab === 'groups' ? <Users size={24} /> : <MessageCircle size={24} />}
            <strong>{tab === 'groups' ? (spaces.length ? 'No group Waves yet' : 'No groups yet') : 'No Waves yet'}</strong>
            <span>
              {tab === 'groups'
                ? (spaces.length ? 'Share an update with a group you are already in.' : 'Join or create a Wavo group first, then its Waves will live here.')
                : "Share something with the people you've added."}
            </span>
            {(tab === 'friends' || spaces.length > 0) && <button onClick={() => setComposerOpen(true)}>Create a Wave</button>}
          </div>
        )}
